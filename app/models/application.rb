class Application < ApplicationRecord
  ...
  def self.get_single_application(app_id)
    select("applications.id, applications.position, sent_date, reference, overall_status, source, notes, companies.title AS company_title, companies.image AS company_image, companies.location AS company_location")
    .joins("LEFT JOIN companies ON companies.id = applications.company_id")
    .joins("LEFT JOIN users ON users.id = applications.user_id")
